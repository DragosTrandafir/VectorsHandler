...
class MyVectorTest(unittest.TestCase):
    def test_create(self):
        ...
        #s6 = MyVector("Vector6", "nocolor", 6, [11, 222, 333, 1, 2, 4])# color is not available
        ...
        #s6.set_nameId("") #we get an error, name_id must be nonempty.
        s6.set_colour("m")
        s6.set_type(1)
        s6.set_values([0,1])
        #self.assertEqual(s6.get_nameId(), "")
        ...
        s3.set_colour("m")
        s3.set_type(1)
        #s3.set_values([]) #we get an error
        #s3.set_values([1]) #we get an error
        s3.set_values([1,0])
        self.assertEqual(s3.get_nameId(), "vectornew")
        self.assertEqual(s3.get_colour(),"m")
        self.assertEqual(s3.get_type(), 1)
        self.assertEqual(s3.get_values(), [1,0])

        try:
            vector=MyVector("Vector3", "b", -1, [3, 4, 88])
            self.assertTrue(False)
        except:
            self.assertTrue(True)

        try:
            vector = MyVector("", "r", 1, [3, 4, 88])
            self.assertTrue(False)
        except:
            self.assertTrue(True)

        try:
            vector = MyVector("Vector3", "r", 1, [])
            self.assertTrue(False)
        except:
            self.assertTrue(True)
